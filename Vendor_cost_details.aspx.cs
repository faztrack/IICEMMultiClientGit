                    ...
                    var result = (from pd in _db.co_pricing_masters
                                  where (from clc in _db.changeorder_locations
                                         ...
                                         (from cs in _db.changeorder_sections
                                          ...
                                          select cs.section_id).Contains(pd.section_level) && pd.estimate_id == EstID && pd.customer_id == Convert.ToInt32(hdnCustomerId.Value) && pd.client_id == Convert.ToInt32(ConfigurationManager.AppSettings["client_id"]) && pd.item_status_id == 1 && pd.is_CommissionExclude == false
