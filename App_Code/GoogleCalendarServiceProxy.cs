...
public class GoogleCalendarServiceProxy
{
    ...
    public IEnumerable<gCalendar> GetCalendars()
    {

        var calendarService = new CalendarService(_authenticator);
        

        var calendars = calendarService.CalendarList.List().Fetch().Items.Select(c => new gCalendar()
             {
                 Id = c.Id,
                 Title = c.Summary,
                 Location = c.Location,
    ...
    public gCalendar GetCalendar(string calendarId)
    {
        //string calid = "phhoopcre034gvlm7v4ndd0m70@group.calendar.google.com";
        var calendarService = new CalendarService(_authenticator);

        var calendar = calendarService.CalendarList.List().Fetch().Items.FirstOrDefault(c => c.Id.Contains(calendarId));
    ...
    public IEnumerable<gCalendarEvent> GetEvents(string calendarId, DateTime startDate, DateTime endDate)
    {
        List<gCalendarEvent> calendarEvents = null;
        var calendarService = new CalendarService(_authenticator);
        ...
        if (result != null)
        {
            calendarEvents = result.Select(c => new gCalendarEvent()
                {
                    Id = c.Id,
    ...
    public gCalendarEvent GetEvent(string calendarId, string eventId)
    {
        var calendarService = new CalendarService(_authenticator);
    ...
    public string CreateEvent(gCalendarEvent calendarEvent)
    {
      //  var cal = GetCalendars();
        var calendarService = new CalendarService(_authenticator);
    ...
    public bool UpdateEvent(gCalendarEvent calendarEvent)
    {
        var calendarService = new CalendarService(_authenticator);
        var calendar = GetCalendar(calendarEvent.CalendarId);
    ...
    public void DeleteEvent(string calendarId, string eventId)
    {
        ...
        var events = calendarService.Events.List(calendarId).SingleEvents;
        //if (events != null)
        //{
            var toUpdate = calendarService.Events.Get(calendarId, eventId).Fetch();
            if (toUpdate.Status != "cancelled")
