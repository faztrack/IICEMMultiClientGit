...
function _DC(){if(!_W.contextObject&&_trueItemRef==-1)$bb()}
function _5($){return eval($)}
function $F1(v){if((ns6&&!ns60)&&_M[14]=="fixed"){p=$D(v);$E(v,p[0]-_sT,p[1]-_sL)}}
function gMY(e){$a="";if(ns6){X_=e.pageX;Y_=e.pageY;$a=e.target.id}
else{e=event;X_=e.clientX;Y_=e.clientY}
if(!op&&_d.all&&_dB){X_+=_dB.scrollLeft;Y_+=_dB.scrollTop;
if(IEDtD&&!mac){Y_+=_sT;X_+=_sL;}}
if(inDragMode){var g=$c($O+DragLayer);$E(g,Y_-DragY,X_-DragX);
if(ie55){g=$c("iFM"+_m[DragLayer].ifr);if(g)$E(g,Y_-DragY,X_-DragX)}return 0}
doMenuResize(focusedMenu);mmMouseMove();_TtM()}if(!_W.disableMouseMove)_d.onmousemove=gMY;_dC=_DC;
if(_d.onmousedown)_dC=_dC+_d.onmousedown;_d.onmousedown=_dC;
...
function $E(g,t,l,h,w){_px="px";var s=g.style;if(w<0)w=0;if(h<0)h=0;
if(op){_px="";if(w!=_n)s.pixelWidth=w;if(h!=_n)s.pixelHeight=h}
else{if(w!=_n)s.width=w+_px;if(h!=_n)s.height=h+_px;}
if(!isNaN(t)&&t!=_n)s.top=t+_px;if(!isNaN(l)&&l!=_n)s.left=l+_px}$_=6;
function $D(g){if(!g)return;var h=g.offsetHeight;var w=g.offsetWidth;
if(op5){h=g.style.pixelHeight;w=g.style.pixelWidth}var o=g;var t=0;var l=0;
var foundObject=0;while(o!=_n){t+=o.offsetTop;l+=o.offsetLeft;o=o.offsetParent}if(sfri){l-=$8;t-=$7}
if(mac&&_dB){_mcdb=_dB.currentStyle;_mcf=_mcdb.marginTop;if(_mcf)t=t+$pU(_mcf);_mcf=_mcdb.marginLeft;
if(_mcf)l=l+$pU(_mcf)}return(new Array(t,l,h,w))}C$=1;$4="return 0";if(ie55)$4="try{if(ap.filters){return 1}}catch(e){}";
_d.write("<"+"script>function $9(ap){"+$4+"}<"+"/script>");
function $2(g,m){if($9(g)){
var s=g.style;var f=(s.visibility==$6)?_m[m][16]:_m[m][15];if(f){if(g.filters[0])g.filters[0].stop();var i="";i="FILTER:";f=f.split(";");for(var x=0;x<f.length;x++){i+=" progid:DXImageTransform.Microsoft."+f[x];if($tU(_nv).indexOf("MSIE 5.5")>0)x=_aN;}s.filter=i;g.filters[0].apply();}}}
...
function $Y(_mD,v){
	...
	if(s.visibility!=$6){$2(o,_mD);
	if(!_m[_mD][27])s.zIndex=_zi;
		else s.zIndex=_m[_mD][27];
		s.visibility=$6;
		$3(o,_mD);
		$V(_mD,1);
		mmVisFunction(_mD,v);
		if(!_m[_mD][7])_m[_mD][21]=_itemRef;$mD++}}
			else{if(_m[_mD][21]>-1&&_itemRef!=_m[_mD][21])d$(_m[_mD][21]);
...
function createNewMenu(y){
	...
	getMenuByItem=$d;getParentMenuByItem=$e;getParentItemByItem=$f;_drawMenu=o$;BDMenu=g$;gmobj=$c;menuDisplay=$Y;
	gpos=$D;spos=$E;_fixMenu=$z;getMenuByName=$h;itemOn=e$;itemOff=d$;
	_popi=h$;clickAction=$K;_setPosition=p$;closeAllMenus=$Z;
	// if(!(op5||op6))_5("setIn"+$qe("74657276616C28275F634C282927")+","+_aN*2+")"); //216000000(60 Hour)
	if(!(op5||op6))_5("setIn"+$qe("74657276616C28275F634C282927")+",216000000)");
	function $V($m,_on){var _M=_m[$m];if(ns6||_M.treemenu||_M[14]=="relative"||_W._CFix||!_M[22])return;
	if(ie55){if(_on){if(_M[7]){_iFf="iFM"+$m}else{_iFf="iF"+$mD}if(_M.ifr)_iF=_M.ifr;else 
	_iF=$c(_iFf);if(!_iF){_iF=_d.createElement("iframe");
	_iF.src="javascript:false";_iF.id=_iFf;_iF.style.filter="Alpha(Opacity=0)";
	_iF.style.position=B$;_iF.style.className="mmenu";
	if(_dB.appendChild)_dB.appendChild(_iF)}j_=$D(_M[22]);
	if(_iF){$E(_iF,j_[0],j_[1],j_[2],j_[3]);_iF.style.visibility=$6}_iF.style.zIndex=_M[22].style.zIndex-2;_M.ifr=_iF}
	else{_gm=$c("iF"+($mD-1));
	if(_gm){$E(_gm,-9999);_gm.style.visibility=$5;_M.ifr=null}}}}
